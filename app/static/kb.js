...
(() => {
  ...
  function paletteItem(b) {
    ...
    item.type = 'button';
    item.draggable = !b.action;
    item.title = b.hint;
    item.innerHTML = `<span class="kb-palglyph">${esc(b.glyph)}</span>`
      + `<span class="kb-pallabel">${esc(b.label)}</span>`;
    if (b.action) {
      // A block whose content depends on the article cannot be dragged as a
      // fixed snippet: which heading it points at is the whole question, and
      // that is decided at the moment of insertion, not before it.
      item.classList.add('kb-palaction');
      item.onclick = () => openAnchorPicker(b);
      return item;
    }
  ...
  /* Jump links: the anchors are generated by the same function the exporter
     uses, so a link written here still lands after the paste. A picker that
     invented its own slugs would produce anchors that work in Tephra and
     nowhere else. */
  async function openAnchorPicker(b) {
    // Flush first. The headings come from the server, and a heading typed
    // ten seconds ago but not yet saved would be missing from the list --
    // the one you most likely want to jump to.
    await flushBody();
    let heads = [];
    try { heads = (await api('/kb/' + S.slug + '/render')).headings || []; } catch {}
    if (!heads.length) {
      return toast('No headings to jump to yet — write one first', 3200);
    }
    if (b.action === 'contents') {
      const list = heads.map((h) =>
        `${h.level > 2 ? '  ' : ''}- [${h.text}](#${h.id})`).join('\n');
      return insertAtLine(caretLine(), list, heads[0].text);
    }
    const menu = el('div', 'kb-anchormenu');
    menu.innerHTML = '<p class="kb-anchorhead">Jump to…</p>';
    heads.forEach((h) => {
      const row = el('button', 'kb-anchorrow' + (h.level > 2 ? ' sub' : ''), h.text);
      row.type = 'button';
      row.onclick = async () => {
        menu.remove();
        await insertAtLine(caretLine(), `[${h.text}](#${h.id})`, h.text);
      };
  ...
  /* The line the caret is on, so an inserted jump link lands where you were
     working rather than at the end of the article. */
  function caretLine() {
    const ta = $('#kbBody');
    if (!ta) return 0;
  ...
  /* ── the live render ── */
  let previewT = null;
  /* The render comes from the server, so the server has to have the text
     first. Saving on this timer rather than waiting for the autosave's own
     is deliberate: the preview debounce is shorter, and a preview that
     rendered the *previous* keystroke would be worse than a slower one. */
