...
(function () {
  ...
  const MARKDOWN_SYNTAX = [
    ...
    { h: 'Tables', ex: '| Left | Right |\n| --- | --- |\n| a | b |\n| c | one<br>two |',
      s: 'Standard GitHub-flavoured markdown tables. A cell cannot contain a newline, so `<br>` inside one breaks the line -- the same thing every tool that generates markdown tables emits. That is the only raw HTML tag that renders; everything else is shown as literal text.' },
    { h: 'Sheets (tabbed tables)', ex: '```sheets\n## Week 1\n| Day | Topic | Reading |\n| --- | --- | --- |\n| Mon | Intro | [[OSI Model]] |\n\n## Week 2\n| Name | Score |\n| --- | --- |\n| Alice | 92 |\n```',
      s: 'A fenced block tagged `sheets` groups several tables into one card with tabs across the top -- the "one spreadsheet, several sheets" shape a single table cannot express, like a week-by-week course plan. Each sheet is a `## Sheet name` heading followed by an ordinary markdown table, and sheets are independent: every one declares its own columns. The stored form is just headings and tables, so it stays readable as plain markdown and edits fine in the note source; hover the card for an Edit button to get a click-and-type grid instead, with Tab/Enter moving between cells and buttons for rows, columns and sheets. Cells are markdown, so `[[wikilinks]]` in them resolve, turn orange when the target does not exist, and count toward backlinks and the graph like any other link. A display-text link inside a table needs its pipe escaped, which the grid editor does for you. Long cells wrap onto more lines rather than stretching their column, so a sheet grows downward instead of scrolling sideways. Drag the right edge of any header cell to set that column’s width, or double-click the edge to hand it back to automatic sizing. The width is stored as the number of dashes in the table’s `| --- |` delimiter row, which is ordinary markdown everywhere else -- GitHub and Obsidian render the table exactly the same, and a formatter that tidies your tables simply resets the columns to automatic. More than one sheet group per note is fine.' },
    ...
    { h: 'Mermaid diagrams', ex: '```mermaid\ngraph TD\n  A[Start] --> B{Decision}\n  B -->|Yes| C[Do it]\n  B -->|No| D[Skip it]\n```',
      s: 'A fenced code block tagged `mermaid` renders as a diagram instead of code -- flowcharts, sequence diagrams, class diagrams, Gantt charts, and everything else Mermaid supports. Same fence syntax as any other language, just with `mermaid` as the tag. It renders at its true natural size, not shrunk to fit the note; click and drag inside it to pan around one too big to show at once. Drag a corner to resize it, the same as an image (and, unlike an image, past the column if you need it bigger) -- the chosen width is saved right on the tag, as `mermaid|500`.' },
    { h: 'Network/cabling diagrams', ex: '```netdiagram\ndevice sw "Switch A" at 40,40 grid\ndevice srv "Server 1" at 320,40\n\nlink sw.ETH1-2 -> srv.ETH1-2 "10G" #A\n```',
      s: 'A fenced code block tagged `netdiagram` draws boxes wired point-to-point by named ports -- built for network/rack/cabling diagrams, not routed through Mermaid. `device id "Label" at x,y` places a box (a dotted id like `rack.fiom0` nests it inside `rack`, no indentation needed); `link a.PORT -> b.PORT` wires two ports, and a numeric range like `ETH1-4 -> ETH1-4` fans out into 4 parallel links drawn as one bundled line. A `#tag` groups links into a color. Add `grid` after a device to draw its ports as a row of labeled cells across its face, like a switch panel -- the ports shown are exactly whichever ones its own `link` lines already reference, nothing to declare separately. Hover the diagram for an Edit button -- a drag-and-drop canvas edits the same underlying text, `at x,y` and `grid` all included.' },
