...
(function () {
  ...
  const MARKDOWN_SYNTAX = [
    ...
    { h: 'Network/cabling diagrams', ex: '```netdiagram\ndevice sw "Switch A" at 40,40\ndevice srv "Server 1" at 320,40\n\nlink sw.ETH1-2 -> srv.ETH1-2 "10G" #A\n```',
      s: 'A fenced code block tagged `netdiagram` draws boxes wired point-to-point by named ports -- built for network/rack/cabling diagrams, not routed through Mermaid. `device id "Label" at x,y` places a box (a dotted id like `rack.fiom0` nests it inside `rack`, no indentation needed); `link a.PORT -> b.PORT` wires two ports, and a numeric range like `ETH1-4 -> ETH1-4` fans out into 4 parallel links drawn as one bundled line. A `#tag` groups links into a color. Hover the diagram for an Edit button -- a drag-and-drop canvas edits the same underlying text, `at x,y` and all.' },
